  ...
  const buscarEventos = async () => {
    try {
      ...
      if (filtrosAtuais.categoriaId)
        params.categoriaId = filtrosAtuais.categoriaId;
      if (filtrosAtuais.statusEventoId)
        params.statusEventoId = filtrosAtuais.statusEventoId;
        ...
        data = data.filter(
          (ev) =>
            ...
            (ev.statusEvento?.idStatusEvento ??
              ev.statusEventoId ??
              ev.statusEvento?.id ??
              ev.status_evento) == filtrosAtuais.statusEventoId
        );
      }
      ...
      if (filtrosAtuais.nome) {
        ...
        filtered = filtered.filter(
          (ev) =>
            String(ev.nomeEvento || ev.nome || ev.nome_evento || "")
              .toLowerCase()
              .includes(q) ||
            String(ev.descricao || "")
              .toLowerCase()
      ...
      const isClosed = (ev) => {
        ...
        if (ids.some((id) => id && (id === "2" || closedStatusIds.has(id))))
          return true;
      ...
      const isOpenOrOngoing = (ev) => {
        ...
        if (
          txt.includes("aberto") ||
          txt.includes("em andamento") ||
          txt.includes("andamento")
        )
          return true;
        const ids = getStatusIds(ev);
        if (ids.some((id) => id && (id === "2" || closedStatusIds.has(id))))
      ...
      try {
        console.debug(
          "[buscarEventos B] total:",
          targetList.length,
          "visible:",
          visible.length,
          "closed total:",
          targetList.filter(isClosed).length
        );
      } catch (e) {
        console.debug("Ignorado:", e);
      }
  ...
  const inscreverEvento = async (idEvento, idStatusInscricao = 1) => {
    ...
    if (!id) {
      Swal.fire({
        ...
        icon: "error",
        confirmButtonText: "OK",
        confirmButtonColor: "#FF4848", // vermelho padrão para erro
      });
      return;
    }
    ...
    try {
      const res = await api.post(
        `/participacoes/${encodeURIComponent(id)}/inscrever`,
        null,
        {
      ...
      if (res.status >= 200 && res.status < 300) {
        ...
        Swal.fire({
          ...
          text: "Você foi inscrito no evento.",
          icon: "success",
          confirmButtonText: "OK",
          confirmButtonColor: "#45AA48", // verde para sucesso
        });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire({
          ...
          ...swalConfirmDefaults,
          reverseButtons: true, // garante que vence o default
        }).then((confirm) => {
