...
export default function EventosBeneficiario() {
  ...
  const buscarEventos = async () => {
    try {
      ...
      const buscarEventos = async () => {
        try {
          ...
          if (
            ...
            endpoint = `/eventos/por-categoria?categoriaId=${encodeURIComponent(
              filtrosAtuais.categoriaId
            )}`;
            ...
            endpoint = `/eventos/status?statusEventoId=${encodeURIComponent(
              filtrosAtuais.statusEventoId
            )}`;
            if (filtrosAdicionais) endpoint += "&" + filtrosAdicionais;
          } else if (filtrosAdicionais) {
              ...
              data = (Array.isArray(data) ? data : []).filter(
                (ev) =>
                  (ev.categoria?.idCategoria ??
                    ev.categoriaId ??
                    ev.categoria?.id) == filtrosAtuais.categoriaId &&
                  (ev.statusEvento?.idStatusEvento ??
                    ev.statusEventoId ??
                    ev.statusEvento?.id) == filtrosAtuais.statusEventoId
              );
            }

            const dataComDadosCompletos = (Array.isArray(data) ? data : []).map(
              (evento) => {
                const categoriaNome =
                  evento.categoria?.nome ||
                  categorias.find(
                ...
                if (enderecoObj && typeof enderecoObj === "object") {
                  const logradouro =
                    ...
                    enderecoObj.logradoro ||
                    "";
                  const numero = enderecoObj.numero
                    ? `${enderecoObj.numero}`
                    : "";
                  const bairro = enderecoObj.bairro
                    ? `${enderecoObj.bairro}`
                    : "";
                  const partes = [];
                  if (logradouro)
      ...
      const computeStatusFromDate = (ev) => {
        ...
        if (!dateStr) return null;
        const start = tryParseDateTime(
          dateStr,
          ev.horaInicio || ev.hora_inicio || ev.hora || ""
        );
        const end =
          tryParseDateTime(
            dateStr,
            ev.horaFim || ev.hora_fim || ev.horaFim || ""
          ) || tryParseDateTime(dateStr, "23:59");
        const now = new Date();
        if (start && end) {
      ...
      const closedStatusIds = new Set(
        (statusList || [])
          ...
          .map((s) => Number(s.idStatusEvento ?? s.id))
          .filter(Boolean)
      );
      ...
      if (userId) {
        try {
          ...
          console.warn(
            "Não foi possível buscar participações do usuário:",
            err
          );
        }
      }
  ...
  const inscreverEvento = async (idEvento, idStatusInscricao = 1) => {
    ...
    const url = `${BASE_URL}/participacoes/${encodeURIComponent(
      id
    )}/inscrever?idUsuario=${encodeURIComponent(
      userId
    ...
    const mostrarDetalhes = async (evento) => {
      ...
      let vagas =
        evento.qtdVaga ?? evento.qtd_vaga ?? "Evento aberto ao público";
      let categoria = evento.categoriaNome || evento.categoria?.nome || "-";
      let status =
        evento.statusSituacao || evento.statusEvento?.situacao || "-";
