    ...
    try {
      ...
      const dataComDadosCompletos = (data || []).map(evento => {
        ...
        const statusSituacao =
          ...
          statusList.find(s => String(s.idStatusEvento) === String(evento.statusEvento?.idStatusEvento ?? evento.statusEventoId ?? evento.statusEvento?.id))?.situacao ||
          "";
      ...
      const processed = (eventosComImg || []).map(ev => {
        ...
        return { ...ev, _startTs: ts || 0, statusEfetivo: effectiveStatus };
      });
