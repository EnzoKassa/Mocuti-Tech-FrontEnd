...
export async function openEventoFormModal(
  ...
  try {
    if ((!categorias || categorias.length === 0)) {
      ...
      if ((!categorias || categorias.length === 0)) {
        try {
          const r = await fetch("/categorias", { method: "GET", headers: { Accept: "application/json", ...getAuthHeaders() }, mode: "cors" });
          if (r.ok) {
  ...
  try {
    if ((!statusList || statusList.length === 0)) {
      ...
      if ((!statusList || statusList.length === 0)) {
        try {
          const r = await fetch("/status-eventos", { method: "GET", headers: { Accept: "application/json", ...getAuthHeaders() }, mode: "cors" });
          if (r.ok) {
            const data = await r.json().catch(() => null);
  ...
  try {
    ...
    if ((!Array.isArray(enderecos) || enderecos.length === 0)) {
      try {
        ...
        const r = await fetch("/endereco/enderecos-eventos", { method: "GET", headers, mode: "cors" });
        if (r.ok) {
          const data = await r.json().catch(() => null);
  ...
  try {
    ...
    if ((!Array.isArray(publicos) || publicos.length === 0)) {
      try {
        const r = await fetch("/eventos/publico-alvo", { method: "GET", headers: { Accept: "application/json", ...getAuthHeaders() }, mode: "cors" });
        if (r.ok) {
          const data = await r.json().catch(() => null);
    ...
    preConfirm: async () => {
      ...
      if (enderecoSelectVal === "__novo") {
        ...
        try {
          ...
          const rEnd = await fetch("/endereco", { method: "POST", headers, body: JSON.stringify(payloadEndereco) });
          if (!rEnd.ok) {
      ...
      try {
        ...
        if (!isEdit) {
          ...
          const res = await fetch("/eventos/cadastrar", {
            method: "POST",
            headers: { ...authHeaders },
            body: fd,
          });
        ...
        if (enderecoId) {
         ...
         const resDados = await fetch(`/eventos/${encodeURIComponent(idEvento)}`, {
           method: "PUT",
        ...
        if (file) {
          const fd = new FormData();
          fd.append("foto", file);
          const resFoto = await fetch(`/eventos/foto/${encodeURIComponent(idEvento)}`, {
            method: "PATCH",
            headers: { ...authHeaders },
            body: fd,
          });
