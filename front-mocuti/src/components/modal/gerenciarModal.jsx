...
export async function openGerenciarModal(evento, { onEdit, onDelete, onLista, navigate, getAuthHeaders } = {}) {
  ...
  const buildLocalString = (ev) => {
    ...
    const looksLikeJsonString = (v) => typeof v === "string" && v.trim().length && ((v.trim().startsWith("{") && v.trim().endsWith("}")) || (v.trim().startsWith("[") && v.trim().endsWith("]")));
    const tryParse = (v) => {
      if (!looksLikeJsonString(v)) return v;
      try { return JSON.parse(v); } catch { return v; }
    };

    const isAddressObject = (o) => o && typeof o === "object" && (o.logradouro || o.endereco || o.rua || o.bairro || o.numero || o.enderecoLogradouro);
    ...
    const findAddress = (node, seen = new Set()) => {
      ...
      if (Array.isArray(node)) {
        ...
        const candidates = ["endereco", "enderecoEvento", "local", "enderecoFormatado", "localizacao", "localObjeto"];
    ...
    if (!addr) {
      const direct = ev.endereco ?? ev.enderecoEvento ?? ev.local ?? ev.enderecoFormatado;
    ...
    if (addr && typeof addr === "object") {
      const log = addr.logradouro || addr.endereco || addr.rua || addr.enderecoLogradouro || "";
      const num = (addr.numero !== undefined && addr.numero !== null) ? String(addr.numero) : (addr.enderecoNumero ? String(addr.enderecoNumero) : "");
    ...
    if (typeof ev.enderecoFormatado === "string" && ev.enderecoFormatado.trim()) return ev.enderecoFormatado.trim();
 ...
 if (!localComputed || !localComputed.trim()) {
  ...
  const imgHtml = imgUrl ? `<img src="${imgUrl}" alt="${escapeHtml(titulo)}" class="sw-img" />` : `<div class="sw-img sw-noimg">Sem imagem</div>`;
    ...
    <div class="sw-modal-compact" style="display:flex; flex-direction:column; gap:18px; position:relative;">
      <div style="display:flex; gap:18px; align-items:flex-start; width:100%;">
        ...
        <div class="sw-right" style="flex:1; min-width:260px; min-height:220px; display:flex; flex-direction:column; justify-content:flex-start;">
          <div style="flex:1;">
            <div class="sw-row"><span class="label">Data:</span><span class="value">${escapeHtml(String(dataFormat))}</span></div>
            <div class="sw-row"><span class="label">Hora:</span><span class="value">${escapeHtml(String(horaInicio))} - ${escapeHtml(String(horaFim))}</span></div>
            <div class="sw-row" style="align-items:flex-start;">
            <span class="label">Local:</span>
            <span class="value" style="white-space:normal; overflow-wrap:break-word;">${escapeHtml(localStr)}</span>
            </div>
            <div class="sw-row"><span class="label">Nº de vagas:</span><span class="value">${escapeHtml(String(vagas))}</span></div>
            <div class="sw-row"><span class="label">Status:</span><span class="value">${escapeHtml(status)}</span></div>
            <div class="sw-row"><span class="label">Categoria:</span><span class="value">${escapeHtml(categoria)}</span></div>
            <div class="sw-row"><span class="label">Público Alvo:</span><span class="value">${escapeHtml(publico)}</span></div>
          </div>
        </div>
      </div>
  ...
  Swal.fire({
    ...
    didOpen: () => {
      ...
      document.getElementById("gerenciar-editar")?.addEventListener("click", () => {
        Swal.close();
      ...
      document.getElementById("gerenciar-dashboard")?.addEventListener("click", () => {
        Swal.close();
